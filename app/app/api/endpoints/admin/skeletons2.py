...
from app.forms import Skeleton2AdminUpdateForm
...
def create_skeleton_category(
    ...
    if category:
        ...
        # num = max(num) + 1
        max_num_category = SkeletonCategoryModel.objects.order_by("-num").first()
        if max_num_category:
            max_num = max_num_category.num or 1
        else:
            max_num = 1
...
def read_skeleton_categories(
    ...
    categories = SkeletonCategoryModel.objects.order_by("-num")[skip: skip + size]
    if categories:
...
async def update_skeleton_category(
    ...
    if swap_with is not None:
        if not isinstance(swap_with, str):
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST,
                                content={"msg": f"invalid swap_with: {swap_with}"})
        #
        swap_with_category = SkeletonCategoryModel.objects(id=swap_with).first()
        if not swap_with_category:
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST,
                                content={"msg": f"Category not found for swap_with: {swap_with}"})
        # To swap two nodes num value
        num_category = category.num
        num_swap_with_category = swap_with_category.num
        category.num, swap_with_category.num = num_swap_with_category, num_category
        #
        category.save()
        swap_with_category.save()
    #
...
async def read_skeleton_audits(
    ...
    code, msg, content = await skeletons_usecase2.read_skeletons(
        menu=skeletons_usecase2.MENU_ADMIN_SKELETON_AUDIT,
...
def put_skeleton_audit(
    ...
    try:
        ar = AuditRecordsModel.objects(
            component=SkeletonModel2.objects(id=skeleton_id).first(),
            audit_type='Tools'
        ).first()
        if ar:
...
async def read_skeletons(
    ...
    # t0 = time.time()
    code, msg, content = await skeletons_usecase2.read_skeletons(
        menu=skeletons_usecase2.MENU_ADMIN_SKELETON,
        background_tasks=background_tasks,
        viewer_id=current_user.id,
        only_own=False,  # View all
        name=name,
...
async def update_skeleton(
        skeleton_id: str,
        update_form: Skeleton2AdminUpdateForm = Depends(),
        current_user: UserModel = Depends(deps.get_current_user)
):
    """
    ...
    if is_online is not None:
        ...
        if state != 'APPROVED':
            return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST,
                                content={
                                    "msg": f"SkeletonModel is `{state}`, must be APPROVED for online/offline operation"})
        else:
            # Cache update add
